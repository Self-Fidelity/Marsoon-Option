...
3. **Databento 不提供官方 Greeks**：其官方路线图（roadmap.databento.com）中"Calculated options greeks schema"仍是未实现的状态项；GLBX.MDP3 的 statistics schema 也不发布结算 IV（stat_type 14）与结算 Delta（stat_type 15）。项目现有的 Black-76 自算引擎（`src/server/barchart-greeks.ts`）在迁移后仍需保留。
...
项目当前数据链路：Barchart 免费延迟源（`scripts/barchart-poller.mjs`）→ `POST /api/ingest/barchart` → 内存仓（`src/server/barchart-store.ts`）→ 落盘 `data/snapshots/YYYY-MM-DD.jsonl` → 五个消费 API（dashboard/levels/chain/term/intraday）。迁移时**消费 API 契约不变**（`src/api/options.ts` 类型为准），只替换采集层。当前消费的数据项：

| # | 数据项 | 当前来源 | 消费方 |
|---|---|---|---|
| 1 | 标的期货报价（last/涨跌/时间） | Barchart quotes get | dashboard 总览 |
| 2 | 期货 1 分钟 K 线（当日 ≤800 根） | Barchart queryminutes | intraday（06）、levels |
| 3 | 到期清单（到期日/类型/剩余天数/PC 量比/波动率） | Barchart expirations get | dashboard、05 热力图 |
| 4 | 期权链逐档（行权价 × 到期 × C/P：bid/ask/last/volume/OI） | Barchart quotes get（逐 serie） | chain（09）、05、07、08 |
| 5 | Greeks/IV/GEX/墙位（自算） | Black-76 引擎 | dashboard、levels、全部 GEX 面板 |

红线沿用 `docs/供应商数据需求.md` §2：缺失一律 `null` 严禁 0 填充、put 侧方向指标自带负号、OI 是快照值不可累加、质量位掩码标记。
...
- **质量标记**：快照需带 `quality_flags` 位掩码（插值/过期/盘口交叉位定义见 `docs/供应商数据需求.md` §2）。
...
关键映射：definition schema 的 `asset` 字段是 parent 符号的根；`underlying` / `underlying_id` 字段把期权挂到标的期货合约——这正好替代 Barchart 源里"周期权挂非主合约期货报价"（如 GC 周期权挂 `GCV26`）的归属逻辑。

### 6.2 schema 与清单字段映射

schema 总表见官方 [Schemas and data formats](https://databento.com/docs/schemas-and-data-formats)。本项目按 L0→L1 选型，明确不需要 L2/L3：

| 清单用途 | Databento schema | 关键字段 | 替代原 Barchart 端点 |
|---|---|---|---|
| A1/A3 到期与链骨架（含 C1/C2/D1 骨架） | `definition`（L0） | `instrument_class`(C/P)、`strike_price`(1e-9 定点)、`expiration`、`activation`、`underlying`、`unit_of_measure_qty`（合约乘数）、`min_price_increment`、`raw_symbol` | expirations get + 链结构 |
| A2/B2/C2 期货 K 线 | `ohlcv-1d` / `ohlcv-1m`（L0） | `open/high/low/close/volume` | queryminutes |
| B1/C1/D1 链逐档 bid/ask | `mbp-1`（L1） | `bid_px_00/ask_px_00`、`bid_sz_00/ask_sz_00`、`action` | quotes get 的 bid/ask |
| B1/C1 成交（last/volume） | `trades`（L1，MBP-1 的 `action='T'` 子集） | `price`、`size`、`side`（主动方） | quotes get 的 last/volume |
| A1/C1/D1 官方 OI / 结算价 | `statistics`（L0） | `stat_type`（3=结算、5/4=时段高/低、6=平仓量、9=持仓）、`price`/`quantity`、`stat_flags` | OI/settlement（原源无） |
| B2 标的期货报价 | `mbp-1` 或 `trades`（期货合约） | 同上 | quotes get 期货报价 |

要点：

- **数据契约适配**：DBN 价格为 1e-9 定点整数，转换浮点时注意精度；`instrument_id` 是 uint32 数值主键，全表 join 靠它。
- **OI 口径升级**：Barchart 的 OI 是页面展示的近似值；Databento 的 OI 是 CME 官方日度统计，按 `ts_ref` 交易日归组、取最后一条，与 `docs/供应商数据需求.md`"OI 是快照值、不可累加"红线一致。
- **逐笔方向（BCR）可行性提升**：`trades`/`mbp-1` 带主动方 `side`（隐含成交有 aggressor），优于 Barchart 完全无方向；但仍存在 `side=N`（如隐含成交）的未知方向，按"计入总量、不计方向拆分"处理。
- **频率决策**：快照聚合层保持项目既定的 1 分钟粒度（`docs/供应商数据需求.md` §2）；L3/L4 用 15–60 分钟粗粒度压低消息量。
...
- 股指现货期权用 Black-Scholes（标的是指数/ETF），与项目现有 Black-76 引擎不同，接入时需加定价模型分支（`docs/技术架构.md` 审计记录 #4：期货期权用 Black-Scholes 是致命错误，反之亦然）。
...
1. **验证期（用 $125 免费额度）**：Python 客户端（`databento` pip 包，`timeseries.get_range`）拉取 `ES.OPT` 的 definition + statistics + `ES.c.0` 的 ohlcv-1m/1d，验证 §六映射表字段齐全性与 1e-9 定点转换，重点验证 A 组收盘核算底稿能否复算出与 Barchart 一致的 wall。
2. **双跑期**：新增 Databento 采集（B 组 0DTE 优先），与 Barchart 采集器并行，向同一 ingest API 推送，比对 Greeks/GEX 结果（同一 Black-76 引擎、双价格源）。
3. **切换期**：Barchart 采集器退役，快照粒度 15 分钟→1 分钟，补齐 levels 历史序列（Databento 历史数据使 levels 面板首次拥有真历史）。
...
- 数据契约红线：`docs/供应商数据需求.md` §2
- 数据契约（接口唯一交互层）：`docs/技术架构.md` §5
- 现行采集器与快照 schema：`scripts/barchart-poller.mjs`、`src/server/barchart-store.ts`
