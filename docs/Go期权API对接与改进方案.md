...
本文以当前代码为依据，历史文档仅作需求参考。此次未修改业务代码、后端配置或数据库，未启动采集器，也没有向行情 ingest 接口 POST 数据。

## 1. 结论与核验边界

建议保留现有 Go Databento Worker、Black-76 引擎与 ClickHouse 数据体系，在其上增加面向看板的查询契约和缺失的数据层。Next.js 保留同源 `/api/options/*` 适配层，负责代理、字段归一化及错误处理；正式 Go 数据不再进入 Barchart ingest 后重算 Greeks。
...
本机接口实测：`dashboard` 返回 `source=local-demo`、33 个 cells；`levels` 返回 `source=local-demo`、39 个 levels；`intraday` 返回 `has_data=false`、空 bars/levels。因此无采集器时并非所有面板都会空态；此前启动说明中的这一表述需要更正。
...
| 全站数据状态 | Barchart capturedAt 变化触发查询失效 | 当前八个 API 没有等价状态协议 | 新 source-neutral status，按 product/scope/数据域输出 revision、更新时间和空态原因 |
...
演示能力如需保留，只放明确进入的独立演示页面；正常数据模式无数据必须显示空态/缺口。06 现有 intraday JSONL 回放与旧 levels 路由不是同一路径，不能把旧 levels 的历史平铺问题误说成 intraday 一定使用假历史。
...
2. `src/app/api/options/*`：改为 Go 适配/只读组合；保留 Barchart 为明确的独立开发来源，不在请求中混合两套引擎结果。
3. `ingest-status.ts` 与查询 hooks：改为 `/api/options/status`；状态版本变化只失效相关 product/scope/domain 的活跃查询。仅替换 dashboard URL 会使当前非 demo 查询停止定时刷新。
...
- `docs/Databento数据源需求.md` C1 写 `[T+30,T+120]`，与现行 d90 的 DTE≤90 不一致；同步五档，不继承旧 L3/L4 编号。
...
- 10 面板“官方 PCR/IV”与 Go 自算来源要重新标注；不沿用 Barchart 的官方标签。
...
| 空周期回退最近系列 | [barchart-analysis](/Users/mima0000/Desktop/Marsoon-Option-main/src/server/barchart-analysis.ts:144) |
...
| 版本刷新依赖 Barchart | [ingest-status](/Users/mima0000/Desktop/Marsoon-Option-main/src/features/options/ingest-status.ts:30) |
