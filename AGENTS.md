...
> **⚠️ 服务器同事开工前必须直接看需求单：`docs/backend/服务端优化与更新策略.md`**（§5 清单 + §7 优先级 + **§8 0DTE 墙位倒挂 R1~R6 + §9 GC 空白实测 + §10 定时预计算/压缩传输方案确认 + §11 部署侧静态资源**，含实测证据、优先级与验收基准）；WS 审查清单另见 `docs/architecture/K线与期权数据周期管理与防堵塞设计.md` §六。下表仅为速览，**不能替代需求单**。
