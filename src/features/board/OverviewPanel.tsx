...
import { MetricCard } from "../options/components/dashboard-ui";
import type { GammaRegime } from "../options/dashboard-view-model";
import { adaptiveLabelIndices } from "./axis-label-density";
import { profileP95 } from "./gex-breakdown-model";
import type { OverviewModel, OverviewProfileBar } from "./overview-model";
import {
  clampStrikeViewport,
...
function exposurePoint(row: OverviewProfileBar, mode: ActiveExposureMode): ExposurePoint {
  if (mode === "oi") {
    return {
      row,
...
/** 第一行以下的全执行价 Exposure 看板：同一张图切换 OI/GEX/DEX/CHEX；可拖拽平移、滚轮缩放。 */
function ExposureDashboard({ model }: { model: OverviewModel }) {
  ...
  const reactId = useId();
  const clipId = `overview-plot-${reactId.replace(/:/g, "")}`;
  ...
  return (
    <section className="flex h-full min-h-0 flex-col overflow-hidden rounded-[10px] border border-[var(--ms-separator)] bg-[var(--ms-panel-bg)]">
      <div className="flex shrink-0 flex-wrap items-center gap-1.5 border-b border-[var(--ms-separator)] px-2 py-2">
      ...
      >
        {model.profile.length === 0 || available.length === 0 ? (
...
/** 01 总览：保留原状态卡第一行，其余区域统一为全执行价 Exposure 看板。 */
export function OverviewPanel({ model }: { model: OverviewModel }) {
  const [measureRef, size] = useMeasureSize<HTMLDivElement>();
  const compact = size.width > 0 && size.width < COMPACT_W;
  const regimeTone =
    model.regime === "positive"
      ? "positive"
      : model.regime === "negative"
        ? "negative"
        : model.regime === "neutral"
          ? "warning"
          : "default";

  return (
    <div ref={measureRef} className="flex h-full min-h-0 flex-col gap-3 overflow-hidden bg-[var(--ms-plot-bg)] p-3">
      <section className="grid shrink-0 grid-cols-2 gap-px overflow-hidden rounded-[10px] border border-[var(--ms-separator)] bg-[var(--ms-separator)] xl:grid-cols-4">
        <MetricCard
          label="Gamma 状态"
          meta={model.scope.toUpperCase()}
          value={regimeLabels[model.regime]}
          tone={regimeTone}
          compact={compact}
        />
        <MetricCard
          label="Spot → Call Wall"
          meta={formatDistance(model.callWall, model.spot)}
          value={
            model.spot !== undefined && model.callWall !== undefined
              ? `${formatPrice(model.spot, model.tickSize)} → ${formatPrice(model.callWall, model.tickSize)}`
              : "--"
          }
          compact={compact}
        />
        <MetricCard
          label="Net GEX（OI 模型）"
          meta="S / 1%"
          value={formatNotional(model.netGEX)}
          tone={(model.netGEX ?? 0) < 0 ? "negative" : "positive"}
          compact={compact}
        />
        <MetricCard
          label="ATM IV"
          meta="SNAPSHOT"
          value={formatPercent(model.atmIV)}
          compact={compact}
        />
      </section>

      <div className="min-h-0 flex-1">
        <ExposureDashboard model={model} />
      </div>
    </div>
  );
}
